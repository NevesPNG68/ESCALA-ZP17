...
WEEKDAYS = {"SEG":"SEGUNDA-FEIRA","TER":"TERCA-FEIRA","QUA":"QUARTA-FEIRA","QUI":"QUINTA-FEIRA","SEX":"SEXTA-FEIRA","SÁB":"SABADO","SAB":"SABADO","DOM":"DOMINGO"}
...
def extract_publication(path: Path, month: int, version: str, source_url: str, imported: str) -> dict:
    records, pending = [], []
    with pdfplumber.open(path) as pdf:
        page_texts = [page.extract_text() or "" for page in pdf.pages]
        names = legend(page_texts[-1]) if page_texts else {}
        published = ""
        match = re.search(r"(\d{1,2}) de ([A-Za-zçÇ]+) de (202[0-9])\s*-\s*(\d{2}:\d{2})", "\n".join(page_texts))
        if match:
        ...
        for page_no, page in enumerate(pdf.pages[:2], start=1):
            ...
            for col, header in enumerate(headers):
                hm = re.match(r"(\d{2})/([A-ZÇ]{3})\s*\n?\s*([A-ZÁÉÍÓÚÇ]{3})", (header or "").upper())
                if not hm: continue
                ...
                for order, row in enumerate(table[1:], start=1):
                    ...
                    for offset, code in enumerate(codes):
                        records.append({"competencia":f"2026-{month:02d}","versao":version,"data":date,"dia_semana":WEEKDAYS.get(weekday,weekday),"ordem":order+offset,
                            "trigrama":code,"nome":names.get(code,""),"situacao":situation,"observacao":"","arquivo_original":path.name,
                            "caminho_arquivo":path.relative_to(BASE).as_posix(),"pagina_original":page_no,"trecho_original":f"{header} | {raw}"})
    return {"competencia":f"2026-{month:02d}","versao":version,"arquivo":path.name,"url_origem":source_url,"data_publicacao":published,"paginas":len(page_texts),"texto_pesquisavel":all(bool(t.strip()) for t in page_texts),"data_importacao":imported,"registros":records,"pendencias":pending}
